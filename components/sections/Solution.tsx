    ...
    >
      ...
      <div className="mt-10 grid grid-cols-1 md:grid-cols-2 gap-4">
        {t.solution.cards.map((card, idx) => (
          <article
            key={idx}
            className="border border-brand-border dark:border-white/10 rounded-lg p-6 bg-white dark:bg-gray-900"
          >
            <h3 className="text-lg font-medium tracking-tight text-brand-neutral dark:text-white">
              {card.title}
            </h3>
            <p className="mt-2 text-brand-muted dark:text-gray-400">
              {card.body}
            </p>
          </article>
        ))}
